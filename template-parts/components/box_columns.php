        ...
        if ($use_categories) {
            ...
            if ($categories): 
                foreach ($categories as $category): 
                    ...
                    $thumbnail_url = wp_get_attachment_image_url( $cat_image, 'medium' );
                    if (!$thumbnail_url) {
            ...
            if ($page_ids) {
                ...
                foreach ($page_ids as $page_id) : 
                    ...
                    $thumbnail_url = get_the_post_thumbnail_url( $page_id, 'medium' );
                    if (!$thumbnail_url) {
                        $thumbnail_url = get_template_directory_uri() . '/images/placeholder-thumbnail.png';
                    }
                    ?>
            ...
            <?php }
                ...
                if ($child_pages):
                    foreach ($child_pages as $child):
                        ...
                        $thumbnail_url = get_the_post_thumbnail_url( $child_id, 'medium' );
                        if (!$thumbnail_url) {
                            $thumbnail_url = get_template_directory_uri() . '/images/placeholder-thumbnail.png';
                        }
                        ?>
