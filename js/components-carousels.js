...
*
*
* Carousel 6 - Step Slider
*
*
*/
document.addEventListener('DOMContentLoaded', function() {
	document.querySelectorAll('.step-slider').forEach((carousel) => {
		// let allowPageScroll = false;

		const stepSlider = new Swiper(carousel, {
			direction: 'vertical',
			slidesPerView: 1,
			mousewheel: {
				releaseOnEdges: true,
		...
		// // Let touch gestures propagate when at the end
		// carousel.addEventListener('touchmove', function (e) {
		// 	console.log('allowPageScroll? ', allowPageScroll);
		// 	if (allowPageScroll) {
		// 		e.stopPropagation(); // Let browser scroll
		// 	}
		// }, { passive: false }); // Ensure default scroll is allowed
	});
});